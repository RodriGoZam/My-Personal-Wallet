...
describe('Unit tests relacionados a agregar un nuevo egreso', () => {
  ...
  it('Agrega el nuevo egreso al inicio del array egresos de la store', () => {
    ...
    wrapper.vm.agregar()
    assert.equal(nuevosDatos[0].cuenta, store.state.egresos[0].cuenta, 'no se agrega la cuenta correctamente')
    assert.equal(nuevosDatos[0].fecha, store.state.egresos[0].fecha, 'no se agrega la fecha correctamente')
    assert.equal(nuevosDatos[0].monto, store.state.egresos[0].monto, 'no se agrega el monto correctamente')
    assert.equal(nuevosDatos[0].categoria, store.state.egresos[0].categoria, 'no se agrega la categoria correctamente')
    assert.equal(90, store.state.cuentas[0].fondos, 'no se actualiza el saldo en la cuenta')
  })
